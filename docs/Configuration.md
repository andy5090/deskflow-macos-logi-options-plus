...
|relativeMouseMoves  | `true` or `false` | If set to ''true'', secondary computers receive relative rather than absolute mouse moves. Deskflow continues tracking its logical cursor for screen switching; while locked to a computer (by Scroll Lock or a configured hot key), it forwards deltas without updating that logical position. This supports games and relative-only clients such as Android UHID. If set to ''false'' or not set, mouse moves are absolute.|
